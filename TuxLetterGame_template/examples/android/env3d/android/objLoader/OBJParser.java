...
public class OBJParser {
	...
	private static HashMap<String, TDModel> modelCache = new HashMap<String, TDModel>(); 
	
	public static TDModel loadObj(AssetManager asset, String fileName){
		if (modelCache.get(fileName) != null) {
			return modelCache.get(fileName);
		} else {
			OBJParser parser = new OBJParser();
			TDModel model = parser.parseOBJ(asset, fileName);
			modelCache.put(fileName, model);
			return model;			
		}		
	}

	public TDModel parseOBJ(AssetManager asset, String fileName) {
		...
		if(faces!= null){//if not this is not the start of the first group
			TDModelPart model=new TDModelPart(faces, vtPointer, vnPointer, m,vn, vt);
			parts.add(model);
		}
